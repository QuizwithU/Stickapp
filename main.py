...
class SmartWalkStickApp(MDApp):
    ...
    def water_data(self):
        ...
        object_names3 = [info['ultrasonic_distance'] for obj_id, info in object_detection_data.items()]
        object_names4 = [info['water_detected'] for obj_id, info in object_detection_data.items()]
        if object_names:
            latest_classname = object_names[-1]
            latest_classname2 = object_names2[-1]
            latest_classname3 = object_names3[-1]
            latest_classname4 = object_names4[-1]
            formatted_number = "{:.2f}".format(latest_classname2)
            formatted_number2 = "{:.2f}".format(latest_classname3)
            ...
            if latest_classname4 == 1:
             self.strng.get_screen('mainscreen').ids.data_label4.text = f"water is Detected"
            else:
             self.strng.get_screen('mainscreen').ids.data_label4.text = f"water is not Detected"
            self.strng.get_screen('mainscreen').ids.data_label5.text = f"Ultra Sonic Distance: {formatted_number2}"
            ...
            text2 = f"and the ultra sonic distance is {formatted_number2}"
            engine = pyttsx3.init()
            engine.say(text2)
            engine.runAndWait()
        ...
        def on_stop(self):
            # Stop the pyttsx3 engine when the application stops
          self.engine.stop()
